...
Cyclone avoids this overhead by making use of directly attached
...
are significantly more expensive than standard DIMMs, which means that they will
likely be available only in smaller quantities compared to flash. We compensate 
for NVDIMM's size by periodically draining the NVDIMM log into a log placed on a
standard SSD on the system.
...
Cyclone uses RAFT protocol~\cite{raft} to keep the logs in sync across the
replicas. On
...
log'' placed in this non-volatile memory.
%% TODO: Messaging has to be improved here. It appears as if the problems
%% with NVDIMM (as far as Cyclone is concerned) will be solved once 3DXPoint
%% appears -- lower cost and higher capacity than DRAM.
While we designed Cyclone to leverage
