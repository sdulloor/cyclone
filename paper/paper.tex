...
availability. Unfortunately, the write ahead log is a performance achilles heel
for these systems - eclipsing much of the work on improving the performance of
the LSM component. To illustrate the impact of the write ahead log, consider
...
value store. Using a replicated write ahead log to build a linearizable and
highly available key value store is not new [find some examples here]. The
novelty in Cyclone is that it accomplishes this without compromising on the
performance baseline of no logging or replication. Cyclone does this by
systematically addressing the sources of performance degradation in
...
RAFT to relieve the CPU of all data copy operations, leaving the core to only
...
parallelism in Cyclone's design in Section~\ref{parallelism}.
