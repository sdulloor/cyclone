...
\usepackage{datetime, url, pdfpages, parskip}
...
\authorinfo{Paper \textbf{\#224}}{13 pages}
...
replicas and we solve the problem as such by using the RAFT protocol implemented
as a software multicast module on the Data Plane Development Kit (DPDK). Second,
...
number of physical two level logs - with a hash of the key being used to choose
...
updates a second using only commodity 10 gigabit ethernet adapters and improves
...
these systems - eclipsing much of the work on improving performance of the LSM
...
performance to drop by an order of magnitude (observe the log scale on the
...
limited amount of NVM reasoning that current forms of NVM such as NVDIMMs are
more expensive than DRAM. Our experiments use an NVM footprint of a mere 64 MB
to achieve performance comparable to placing the entire log in NVM. This allows
Cyclone to be used in deployments where the key value store is maintained
entirely in main memory rather than as a multi-level LSM tree structure, with
the log serving as the source for recovery in the event of failure.
...
We note that by discrding stream oriented protocols such as TCP, the network
layer is not longer reliable or ordered. This is not a problem for RAFT that is
...
decoupled from execution and thus multiple updates are usually in
flight on the network for replication at the same time.
...
Our solution instead is to use a technique we termed ganged operations. It is
...
logs). The injection core uses DPDKs packet cloning primitives to avoid
...
RAFT instances (read from shared memory of the co-located leaders).  We describe
the remaining flow for ganged operations in terms of the pseudocode in
Figure~\ref{fig:ganged_ops}. We defer discussion of how we generate the nonce to
later in this section. We also assume that a unique barrier is allocated in
...
store cores execute \\{\tt event\_replicated\_ganged\_op}. They use a bitmask for
...
core (we use the minumum numbered participating KV store core) is
...
one of the non-leader cores could fail. The
...
view from the leader core in shared memory. We add that on a leadership change
due to timeout we enqueue and execute a nop on the physical log and all
associated KV store cores to ensure previous entries are committed - a corner
case in the RAFT protocol.
...
this affects the ability to simultaneously issues replication messages on all
...
extra round trip delay (including replication for all phases of a 2PC), which
would the minimum needed for a solution using distributed transactions via two
phase commit.
...
request if currently left to the key value store and client, since we felt that
...
\section{Conclusion}
Persistent key values stores struggle today to provide high availability using a
...
will help key value store builders and researchers deal with their logging
problem.
