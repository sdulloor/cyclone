...
replicate as many as 8 million updates a second using just 4 commodity 10 GigE
ports on a machine.
...
client machine has one 82599-based ethernet port. All the machines are connected
through the same commodity 10 Gbit switch. We use 8 cores on the server machines
...
\emph{8 million messages a second}. In the case of RPC calls with 512 byte
...
the responses to the clients, originating from different machines have access to
...
on top of NVML. NVML is a general purpose library written for a variety of
