...
IO for every update. For small to moderately sized updates, this leads to severe
write amplification as a 4KB page must be synchronously written to the log file
on flash. Cyclone avoids this by making use of directly attached persistent
...
Using non-volatile memory to cache writes to flash is also employed in the
internal architecture of flash SSDs - including the SSD we use in the evaluation
of the paper. This ``write cache'' however does not help with small appends to
the write ahead log, as experiments with Rocksdb's own write-ahead log has shown
in this paper.
